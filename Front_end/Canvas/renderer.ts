...
export async function createTextureFromArrayBuffer(
    gl: WebGLRenderingContext,
    arrayBuffer: ArrayBuffer
): Promise<WebGLTexture> {

    // 1. Wrap raw bytes into a Blob
    const blob = new Blob([arrayBuffer]);

    // 2. Convert Blob → ImageBitmap (fast GPU-friendly decode)
    ...
    // 6. Texture parameters (IMPORTANT for non-power-of-two images)
