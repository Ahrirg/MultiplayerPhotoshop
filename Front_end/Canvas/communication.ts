...
export function initWebsocketWrapper(serverIP: string) {
  ws = new WebsocketWrapper(serverIP, "canvas", (event) =>
    handleServerMessage(event),
  );

  ws.parseHistory(`${serverIP}/api/canvas/history`);
}

export function sendObjectModificationMessage(
  changes: Partial<Obj>,
  objectId: string,
) {
  ws.sendMessage({
